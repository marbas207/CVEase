...
/**
 * Rewrite a CVE's most recent stage transition to be `daysAgo` days in the
 * past. Used to make demo CVEs show realistic time-in-stage values and to
 * naturally produce a couple of "stale" entries on the dashboard.
 */
async function backdateStage(cveId: string, daysAgo: number): Promise<void> {
  const ts = new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString()
  await api.cve.backdateStage(cveId, ts)
}

/** Date `daysFromNow` days from today as a YYYY-MM-DD string. */
...
- **Patient safety implications**: this is a Class III medical device`,
    ...
    date_discovered: dateOffset(-14),
    date_vendor_notified: dateOffset(-12)
  })
  await api.cve.move(alarisOverflow.id, 'Vendor Contacted', alaris.id, 0)
  // Follow-up is OVERDUE by 2 days — this is the demo case for the OS
  // notification scheduler. On launch with demo data the user gets a real
  // OS notification for this CVE.
  await api.cve.update(alarisOverflow.id, { followup_due_date: dateOffset(-2) })
  // 12 days in Vendor Contacted — visible in the time-in-stage display but
  // not yet stale.
  await backdateStage(alarisOverflow.id, 12)
  ...
  const pyxisEnum = await api.cve.create({
    ...
    date_discovered: dateOffset(-65),
    date_vendor_notified: dateOffset(-60)
  })
  ...
  // 60 days in Negotiating + no follow-up date set = STALE. This is the
  // "BD ghosted you" story arc and is the canonical stale-CVE demo case.
  await backdateStage(pyxisEnum.id, 60)
  ...
  const wordMacro = await api.cve.create({
    ...
    date_discovered: dateOffset(-22),
    date_vendor_notified: dateOffset(-18)
  })
  ...
  await api.cve.update(wordMacro.id, { followup_due_date: dateOffset(14) })
  // 18 days in Negotiating — gives the time-in-stage display some variety
  // alongside the alarisOverflow / pyxisEnum / adPrivEsc spread.
  await backdateStage(wordMacro.id, 18)
...
\`\`\``,
    ...
    references_list: [
      ...
      'https://posts.specterops.io/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-93565be8b7d3'
    ].join('\n'),
    date_discovered: dateOffset(-35),
    date_vendor_notified: dateOffset(-30),
    date_cve_requested: dateOffset(-25)
  })
  ...
  await api.cve.update(adPrivEsc.id, { followup_due_date: dateOffset(7) })
  // 25 days in CVE Requested — under the stale threshold, follow-up active.
  await backdateStage(adPrivEsc.id, 25)
  ...
  const excelFormulaInj = await api.cve.create({
    ...
    date_discovered: dateOffset(-120),
    date_vendor_notified: dateOffset(-115),
    date_cve_requested: dateOffset(-90),
    date_disclosed: dateOffset(-45)
  })
  ...
  await api.cve.update(excelFormulaInj.id, {
    ...
    bounty_status: 'paid',
    bounty_amount: '$5,000',
    bounty_paid_date: dateOffset(-30)
  })
...
This is **not CVE-eligible** under most scoring frameworks (it's a model behavior issue, not a software vulnerability) but is in scope for Google's bug bounty.`,
    ...
    date_discovered: dateOffset(-50),
    date_vendor_notified: dateOffset(-45),
    cve_eligible: 0
  })
  ...
  // 45 days in Negotiating + no follow-up date = STALE. Second canonical
  // stale-CVE demo case — Google "still investigating" with no scheduled
  // touchpoint.
  await backdateStage(geminiPromptInj.id, 45)
  ...
  const gmailXss = await api.cve.create({
    ...
    date_discovered: dateOffset(-95),
    date_vendor_notified: dateOffset(-90),
    date_disclosed: dateOffset(-83),
    cve_eligible: 0
  })
  ...
  await api.cve.update(gmailXss.id, {
    ...
    bounty_paid_date: dateOffset(-80),
  ...
  const adNtlm = await api.cve.create({
    ...
    date_discovered: dateOffset(-180),
    date_vendor_notified: dateOffset(-175),
    date_cve_requested: dateOffset(-130),
    date_disclosed: dateOffset(-60)
  })
  ...
  await api.cve.update(adNtlm.id, {
    ...
    bounty_paid_date: dateOffset(-44)
  })

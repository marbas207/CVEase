import { AlertCircle, Clock, BellRing, Hourglass } from 'lucide-react'
import { ActionCard } from './ActionCard'
import { formatDate, daysUntil } from '../../lib/utils'
import { daysInCurrentStage } from '../../lib/stageDuration'
...
interface Props {
  ...
  /**
   * CVEs that have been sitting in the same in-flight stage for too long
   * without an active follow-up reminder. The "you forgot about this"
   * surface — see lib/stageDuration.ts for the threshold.
   */
  staleCVEs: CVEWithContext[]
...
export function ActionItemsGrid({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {/* Stale CVEs — top-left of the grid because this is the "you forgot
          about this" surface and the most likely to need immediate action
          when populated. Only renders when there's something to show. */}
      {staleCVEs.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2 text-orange-400">
            <Hourglass className="w-4 h-4" />
            Stale Vulnerabilities
            <span className="text-xs bg-orange-500/20 px-1.5 py-0.5 rounded">{staleCVEs.length}</span>
          </h3>
          <div className="space-y-2">
            {staleCVEs.map(({ cve, vendor, swimlane }) => {
              ...
              return (
                <ActionCard
                  ...
                  detail={`In ${cve.stage} for ${days} day${days === 1 ? '' : 's'} with no active follow-up`}

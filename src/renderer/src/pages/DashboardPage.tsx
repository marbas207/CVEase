...
import { isStaleCVE, daysInCurrentStage } from '../lib/stageDuration'
...
export function DashboardPage() {
  ...
  // CVEs that have been sitting in the same stage too long, sorted by oldest
  // first. Excludes anything already covered by the overdue / follow-up
  // sections so the dashboard doesn't double-count the same items.
  const overdueIds = new Set(overdueDeadlines.map((c) => c.cve.id))
  const followupOverdueIds = new Set(followupsOverdue.map((c) => c.cve.id))
  const staleCVEs = cves
    .filter((c) => isStaleCVE(c))
    .filter((c) => !overdueIds.has(c.id) && !followupOverdueIds.has(c.id))
    .sort((a, b) => (daysInCurrentStage(b) ?? 0) - (daysInCurrentStage(a) ?? 0))
    .map(enrich)
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Your vulnerability disclosure pipeline at a glance.</p>
      </div>

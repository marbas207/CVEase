    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          {/* Created / updated / time-in-stage metadata */}
          {(() => {
            const days = daysInCurrentStage(cve)
            const stageDuration = days === null
              ? null
              : days === 0
                ? 'today'
                : days === 1
            ...
            return (
              <div className="text-xs text-muted-foreground">
                ...
                {stageDuration && (
                  <>
                    {' · '}
                    <span>In <strong className="text-foreground">{cve.stage}</strong> for {stageDuration}</span>
                  </>
                )}
              </div>
            )
          ...
          {/* References — newline-separated URL list. Trim, drop empties,
              and render each as a clickable external link. */}
          {(() => {
            const refs = (cve.references_list ?? '')
              .split(/\r?\n/)
              .map((s) => s.trim())
              .filter((s) => s.length > 0)

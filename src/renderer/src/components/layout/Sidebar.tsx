import { Gauge, Columns3, Clock, Settings, Bug, Trophy, Sun, Moon, Info } from 'lucide-react'
  ...
  return (
    <aside className="w-[72px] bg-sidebar border-r border-sidebar-border flex flex-col items-center pt-8 pb-3 gap-0.5 shrink-0">
      ...
      >
        ...
        <span className="text-[10px] font-bold text-primary tracking-tight">CVEase</span>
      </button>
      ...
      {navItems.map(({ id, label, Icon }) => (
        <button
          ...
          title={label}
          className={cn(
            'relative flex flex-col items-center gap-0.5 w-[60px] py-1.5 rounded-lg transition-colors',
            currentPage === id
      ...
      >
        ...
        <span className="text-[10px] leading-none">About</span>
      </button>
      <button
        onClick={toggle}
        title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}
        className="flex flex-col items-center gap-0.5 w-[60px] py-1.5 rounded-lg text-sidebar-foreground hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground transition-colors"
      >
        {theme === 'dark'
          ? <Sun className="w-[18px] h-[18px]" />
          : <Moon className="w-[18px] h-[18px]" />}
        <span className="text-[10px] leading-none">{theme === 'dark' ? 'Light' : 'Dark'}</span>
      </button>
    </aside>
  )
}

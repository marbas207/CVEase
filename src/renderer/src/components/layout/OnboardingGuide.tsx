...
export function OnboardingGuide() {
  ...
  const handleLoadDemo = async () => {
    ...
    try {
      ...
      // Surface the failure in the UI rather than silently swallowing it.
      // Previously a thrown IPC error here looked identical to a no-op
      // click, which made debugging the demo loader awful.
    ...
    <>
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="max-w-lg w-full space-y-8">
          {/* Header — full logo, theme-aware */}
          <div className="text-center space-y-3">
            <div className="flex justify-center">
              <img src={logoLight} alt="CVEase" className="block dark:hidden w-64" />
              <img src={logoDark} alt="CVEase" className="hidden dark:block w-64" />
            </div>
            <p className="text-sm text-muted-foreground leading-relaxed">
              Welcome. Let's set up your first disclosure workflow in three quick steps.
            </p>
          </div>
          ...
          {demoError && (
            <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-3 text-xs text-destructive">
              <p className="font-semibold mb-1">Demo data failed to load</p>
              <p className="font-mono break-all">{demoError}</p>
            </div>
          )}

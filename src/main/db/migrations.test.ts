...
describe('database migrations', () => {
  ...
  it('includes columns added by later migrations', () => {
    ...
    // v16: cve_stage_history table
    expect(columnNames(db, 'cve_stage_history')).toEqual(
      expect.arrayContaining(['id', 'cve_id', 'stage', 'changed_at'])
    )
    // v17: references_list
    expect(columnNames(db, 'cves')).toContain('references_list')
  })

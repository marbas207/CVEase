...
export function listCVEs(filters?: CVEFilters): CVERow[] {
  ...
  return db.prepare(`SELECT * FROM cves ${where} ORDER BY swimlane_id, stage, sort_order ASC`).all(...values) as CVERow[]
}

export function listArchivedCVEs(): CVERow[] {
  return getDb()
    .prepare('SELECT * FROM cves WHERE archived = 1 ORDER BY archived_at DESC')
    .all() as CVERow[]
}
...
export function listArchiveEligible(): CVERow[] {
  ...
  return getDb()
    .prepare(`SELECT * FROM cves WHERE archived = 0 AND stage = 'Published' AND date_disclosed IS NOT NULL AND date_disclosed <= ?`)
...
export function getCVE(id: string): CVERow | null {
  return getDb().prepare('SELECT * FROM cves WHERE id = ?').get(id) as CVERow | null
}

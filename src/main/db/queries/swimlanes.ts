...
export function createSwimlane(input: CreateSwimlaneInput): SwimlaneRow {
  ...
  db.prepare(`
    INSERT INTO swimlanes (id, software_name, vendor, vendor_id, version_affected, url, vendor_is_cna, bounty_in_scope, sort_order)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, input.software_name, input.vendor, input.vendor_id ?? null, input.version_affected ?? null, input.url ?? null, input.vendor_is_cna ? 1 : 0, input.bounty_in_scope ? 1 : 0, maxOrder + 1)
...
export function updateSwimlane(id: string, input: UpdateSwimlaneInput): SwimlaneRow {
  ...
  if (input.bounty_in_scope !== undefined) { fields.push('bounty_in_scope = ?'); values.push(input.bounty_in_scope ? 1 : 0) }

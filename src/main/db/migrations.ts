...
export function runMigrations(db: Database.Database): void {
  ...
  if (currentVersion < 16) {
    // Stage transition history. One row per stage change so we can compute
    // "this CVE has been in Vendor Contacted for 73 days" reliably, surface
    // stale CVEs on the dashboard, and (eventually) build proper analytics.
    //
    // We seed an initial row for every existing CVE using its current stage
    // and its created_at timestamp — that's the best approximation we have
    // for CVEs that pre-date this migration. Time-in-stage will be slightly
    // wrong for any pre-v16 CVE that was moved between stages, but it'll
    // self-correct from the next stage change forward.
    db.exec(`
      CREATE TABLE IF NOT EXISTS cve_stage_history (
        id          TEXT PRIMARY KEY,
        cve_id      TEXT NOT NULL REFERENCES cves(id) ON DELETE CASCADE,
        stage       TEXT NOT NULL CHECK(stage IN (
          'Discovery','Vendor Contacted','Negotiating','CVE Requested','Published'
      ...
      CREATE INDEX IF NOT EXISTS idx_cve_stage_history_cve ON cve_stage_history(cve_id);
    `)

    const seedStmt = db.prepare(
      'INSERT INTO cve_stage_history (id, cve_id, stage, changed_at) VALUES (?, ?, ?, ?)'
    )
    const existing = db
      .prepare('SELECT id, stage, created_at FROM cves')
      .all() as { id: string; stage: string; created_at: string }[]
    const seedAll = db.transaction(() => {
      for (const cve of existing) {
        seedStmt.run(uuidv4(), cve.id, cve.stage, cve.created_at)
      }
    })
    seedAll()
  ...
  if (currentVersion < 17) {
    // Free-form references list. Stored as a single newline-separated text
    // blob — same simplicity argument as tags. The detail panel splits on
    // newlines, trims, drops empties, and renders each as a clickable link.
    // Existing patch_url and bounty_url stay as their own first-class fields
    // because they have specific semantics elsewhere (advisory drafts, etc.).
    addColumn(db, 'cves', 'references_list', 'TEXT')

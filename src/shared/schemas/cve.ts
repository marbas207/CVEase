...
export const CreateCVEInput = z
  .object({
    ...
    references_list: z.string().optional(),
...
export const UpdateCVEInput = z
  .object({
    ...
    references_list: z.string().nullable().optional(),
...
/**
 * Args tuple for `cve:backdateStage`. The timestamp must be a full ISO 8601
 * datetime string, not just a date — SQLite stores stage history with
 * datetime precision and the renderer compares against `Date.now()`.
 */
export const BackdateStageArgs = z.tuple([
  IdString,
  z.string().datetime()
])
